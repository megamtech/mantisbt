...
function layout_page_header_begin($p_page_title = null) {
    ...
    foreach ($t_searches as $t_type) {
        ...
        'title="' . sprintf(lang_get("opensearch_{$t_type}_description"),
                $t_title) . '" ',
        'href="' . string_sanitize_url('browser_search_plugin.php?type=' . $t_type,
                true) .
        '"/>',
        "\n";
    }
...
function layout_login_page_begin() {
    ...
    # Advertise the availability of the browser search plug-ins.
    echo "\t", '<link rel="search" type="application/opensearchdescription+xml" title="MantisBT: Text Search" href="' . string_sanitize_url('browser_search_plugin.php?type=text',
            true) . '" />' . "\n";
    echo "\t", '<link rel="search" type="application/opensearchdescription+xml" title="MantisBT: Issue Id" href="' . string_sanitize_url('browser_search_plugin.php?type=id',
            true) . '" />' . "\n";
...
function layout_navbar_user_menu($p_show_avatar = true) {
    ...
    if (!current_user_is_protected()) {
        layout_navbar_menu_item(helper_mantis_url('account_page.php'),
                lang_get('account_link'), 'fa-user');
    }

    # RSS Feed
    if (OFF != config_get('rss_enabled')) {
        layout_navbar_menu_item(htmlspecialchars(rss_get_issues_feed_url()),
                lang_get('rss'), 'fa-rss-square orange');
    }

    echo '<li class="divider"></li>';

    # Logout
    layout_navbar_menu_item(helper_mantis_url('logout_page.php'),
            lang_get('logout_link'), 'fa-sign-out');
    echo '</ul>';
    echo '</li>';
...
function layout_navbar_projects_menu() {
    ...
    if ($t_show_project_selector) {
        ...
        if (ALL_PROJECTS == $t_current_project_id) {
            ...
            echo '&#160;' . string_attribute(project_get_field($t_current_project_id,
                            'name')) . '&#160;' . "\n";
        }
        ...
        layout_navbar_projects_list(join(';', helper_get_current_project_trace()),
                true, null, true);
        echo '</ul>' . "\n";
        echo '</li>' . "\n";
    } else {
        ...
        if (ALL_PROJECTS == helper_get_current_project()) {
            ...
            $t_redirect_url = str_replace(config_get('short_path'), '',
                    $_SERVER['REQUEST_URI']);
...
function layout_navbar_projects_list($p_project_id = null, $p_include_all_projects = true, $p_filter_project_id = null, $p_trace = false, $p_can_report_only = false) {
    ...
    foreach ($t_project_ids as $t_id) {
        if ($p_can_report_only) {
            $t_report_bug_threshold = config_get('report_bug_threshold', null,
                    $t_user_id, $t_id);
            $t_can_report = access_has_project_level($t_report_bug_threshold,
                    $t_id, $t_user_id);
        }
        ...
        layout_navbar_subproject_option_list($t_id, $p_project_id,
                $p_filter_project_id, $p_trace, $p_can_report_only);
    }
...
function layout_navbar_subproject_option_list($p_parent_id, $p_project_id = null, $p_filter_project_id = null, $p_trace = false, $p_can_report_only = false, array $p_parents = array()) {
    ...
    foreach ($t_project_ids as $t_id) {
        ...
        layout_navbar_subproject_option_list($t_id, $p_project_id,
                $p_filter_project_id, $p_trace, $p_can_report_only, $p_parents);
    }
...
function layout_print_sidebar($p_active_sidebar_page = null) {
    if (auth_is_user_authenticated()) {
        ...
        layout_plugin_menu_options_for_sidebar($t_event_menu_options,
                $p_active_sidebar_page);

        # Main Page
        if (config_get('news_enabled') == ON) {
            layout_sidebar_menu('main_page.php', 'main_link', 'fa-bullhorn',
                    $p_active_sidebar_page);
        }

        # My View
        layout_sidebar_menu('my_view_page.php', 'my_view_link', 'fa-dashboard',
                $p_active_sidebar_page);

        # View Bugs
        layout_sidebar_menu('view_all_bug_page.php', 'view_bugs_link',
                'fa-list-alt', $p_active_sidebar_page);

        # Report Bugs
        if (access_has_project_level(config_get('report_bug_threshold'))) {
            $t_bug_url = string_get_bug_report_url();
            layout_sidebar_menu($t_bug_url, 'report_bug_link', 'fa-edit',
                    $p_active_sidebar_page);
        }

        # Changelog Page
        if (access_has_project_level(config_get('view_changelog_threshold'))) {
            layout_sidebar_menu('changelog_page.php', 'changelog_link',
                    'fa-retweet', $p_active_sidebar_page);
        }

        # Roadmap Page
        if (access_has_project_level(config_get('roadmap_view_threshold'))) {
            layout_sidebar_menu('roadmap_page.php', 'roadmap_link', 'fa-road',
                    $p_active_sidebar_page);
        }

        # Summary Page
        if (access_has_project_level(config_get('view_summary_threshold'))) {
            layout_sidebar_menu('summary_page.php', 'summary_link',
                    'fa-bar-chart-o', $p_active_sidebar_page);
        }

        # Project Documentation Page
        if (ON == config_get('enable_project_documentation')) {
            layout_sidebar_menu('proj_doc_page.php', 'docs_link', 'fa-book',
                    $p_active_sidebar_page);
        }

        # Project Wiki
        if (ON == config_get_global('wiki_enable')) {
            layout_sidebar_menu('wiki.php?type=project&amp;id=' . $t_current_project,
                    'wiki', 'fa-book', $p_active_sidebar_page);
        }

        # Manage Users (admins) or Manage Project (managers) or Manage Custom Fields
        if (access_has_global_level(config_get('manage_site_threshold'))) {
            layout_sidebar_menu('manage_overview_page.php', 'manage_link',
                    'fa-gears', $p_active_sidebar_page);
        } else {
            $t_show_access = min(config_get('manage_user_threshold'),
            ...
            if (access_has_global_level($t_show_access) || access_has_any_project($t_show_access)) {
                ...
                if (access_has_global_level(config_get('manage_user_threshold'))) {
                    ...
                    if (access_has_project_level(config_get('manage_project_threshold'),
                                    $t_current_project) && ( $t_current_project <> ALL_PROJECTS )) {
        ...
        if (config_get('time_tracking_enabled') && access_has_global_level(config_get('time_tracking_reporting_threshold'))) {
            layout_sidebar_menu('billing_page.php',
                    'time_tracking_billing_link', 'fa-clock-o',
                    $p_active_sidebar_page);
        }
...
function layout_options_for_sidebar($p_menu_options, $p_active_sidebar_page) {
    foreach ($p_menu_options as $t_menu_option) {
        ...
        layout_sidebar_menu($t_menu_option['url'], $t_menu_option['title'],
                $t_icon, $p_active_sidebar_page);
    }
...
function layout_breadcrumbs() {
    ...
    $t_protected = current_user_get_field('protected');
    $t_access_level = get_enum_element('access_levels',
...
function layout_footer() {
    ...
    if (!is_page_name('login_page')) {
        $t_webmaster_contact_information = sprintf(lang_get('webmaster_contact_information'),
                string_html_specialchars(config_get('webmaster_email')));
    ...
    if (config_get('show_timer')) {
        $t_page_execution_time = sprintf(lang_get('page_execution_time'),
                number_format(microtime(true) - $g_request_time, 4));
        echo '<small><i class="fa fa-clock-o"></i> ' . $t_page_execution_time . '</small>&#160;&#160;&#160;&#160;' . "\n";
    }

    # Print the page memory usage
    if (config_get('show_memory_usage')) {
        $t_page_memory_usage = sprintf(lang_get('memory_usage_in_kb'),
    ...
    if (config_get('show_queries_count')) {
        ...
        $t_total_queries_executed = sprintf(lang_get('total_queries_executed'),
        ...
        if (config_get_global('db_log_queries')) {
            $t_unique_queries_executed = sprintf(lang_get('unique_queries_executed'),
                    $t_unique_queries_count);
            echo '<small><i class="fa fa-database"></i> ' . $t_unique_queries_executed . '</small>&#160;&#160;&#160;&#160;' . "\n";
        }
        $t_total_query_time = sprintf(lang_get('total_query_execution_time'),
                $t_total_query_execution_time);
